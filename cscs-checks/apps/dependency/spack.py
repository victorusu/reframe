...
# SPECS = [[r'gromacs@2020.2 %gcc@8.3.0', '~mpi ~shared build_type=Release ^fftw ~mpi']]
SPECS = [
    ['pkg-config@0.29.2', 'default'],
    ['zlib@1.2.11', 'default'],
    [r'gromacs@2020.2', '~mpi ~shared build_type=Release ^fftw ~mpi']
]
...
SPECS_COMPILERS_SPACK = ([d,m] for d in SPECS_COMPILERS for m in SPACK)
SPECS_COMPILERS_SPACK = ([x[0][0], x[0][1], x[1][0], x[1][1]] for x in SPECS_COMPILERS_SPACK)

SPECS_SPACK = ([x,y,z] for x in SPECS for y in SPACK for z in SPECS_COMPILERS)
SPECS_SPACK = ([x[0][0], x[0][1], x[1][0], x[1][1], x[2][0], x[2][1]] for x in SPECS_SPACK)
...
@rfm.parameterized_test(*SPECS_COMPILERS_SPACK)
# should be rfm.RegressionTest but simplifying for the test
class SpackCompilerInstall(rfm.RunOnlyRegressionTest):
    def __init__(self, compiler, compiler_spec, spack_version, spack_path):
        self.compiler = compiler
        if compiler_spec in [None, 'default']:
            self.compiler_spec = ''
        else:
            self.compiler_spec = compiler_spec

        if spack_path in ['stage']:
            self.spack_path = 'stage'
        elif spack_path in ['output']:
            self.spack_path = 'stage'
        ...
        if self.current_system.name in ['daint', 'dom']:
            self.executable_opts = ['compiler', 'list']
        else:
            self.executable_opts = ['install', f'{self.compiler}', f'{self.compiler_spec}']
            self.postrun_cmds += [
                # removing the
                f'spack compiler add `spack location -i {self.compiler}`',
                f'spack compiler add `spack location -i {self.compiler}`/bin/linux-x86',
                f'spack compiler add `spack location -i {self.compiler}`/linux-x86/bin',
            ]
        ...
        self.sanity_patterns = sn.all([
            ...
            sn.assert_not_found('Errno', self.stderr),
            sn.assert_not_found('See build log for details', self.stderr),
            sn.assert_found(f'{compiler}', self.stdout),
        ])
...
@rfm.parameterized_test(*SPECS_SPACK)
# should be rfm.RegressionTest but simplifying for the test
class SpackPackageInstall(rfm.RunOnlyRegressionTest):
    def __init__(self, pkg, spec, spack_version, spack_path, compiler, compiler_spec):
        self.pkg = pkg
        ...
        self.dep_name = f'SpackCompilerInstall_{util.toalphanum(compiler)}_{util.toalphanum(compiler_spec)}_{util.toalphanum(spack_version)}_{util.toalphanum(spack_path)}'
